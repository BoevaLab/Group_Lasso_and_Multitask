...
#	sigma = 0.005
	for jj in range(rpoints):
		for cc in range(nc):
			notdone_temp = True
			sigma = 0.1*lambest0[cc]  # allow variations within 10%
#			print('sigma = '+str(sigma))
	...
	return lamdata_rand
###################################################################################
def mdata(rpoints,nc):   # generate mu data: random
	ii = 0
	mu_max = .5 #1/2;
	mudata_rand = []
	while ii<rpoints:
...
#					ttemp = np.abs(np.random.normal(0,mu_max/3,1)) # random normal dist
					ttemp = np.abs(np.random.normal(0,mu_max,1)) # random normal dist
				mu_temp[ll,kk] = ttemp;
...
for cancers_type in cancers:
	...
	visualisation = False  # visualise betas for 2 cancers
	...
	rpoints = 15#30#30#30 #30 #50   # nr of random points
	
	# select best
	nrv = 100 #50 #1#5 #20 #30 #5#30#20#20#1#5 #5 #5 # 20  # how many validation sets
	...
	epoch = 300 #400  # max nr of epochs
	...
	#####				
			...
			mudata_rand = mdata(rpoints,nc)  # random mu samples between 0 and 0.5
...
#					rgba = cmap(0.5)
					...
					plt.plot(betabest_single[cc],'o')#, beta22+beta3,'*')
#					plt.plot(betabest_final[cc],'o', beta1+beta21,'*')
